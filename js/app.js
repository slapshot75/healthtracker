...
function exportData() {
  const payload = {
    ...
    today:       JSON.parse(localStorage.getItem(STORAGE_KEY)      || '[]'),
    history:     JSON.parse(localStorage.getItem(HISTORY_KEY)      || '[]'),
    walkHistory: JSON.parse(localStorage.getItem(WALK_HISTORY_KEY) || '[]'),
  };
  ...
  a.href     = url;
  a.download = `purin-daten-${date}.json`;
  a.click();
...
function importData(event) {
  ...
  reader.onload = e => {
    try {
      ...
      if (!payload.version || (!payload.history && !payload.today)) {
        showToast('⚠ Ungültige Datei – kein gültiges Export-Format', 3000);
        return;
      }
      const existingHistory = getHistory();
      const importHistory   = payload.history || [];

      // Merge: import adds new days, existing days are kept unless same timestamp
      const merged = [...existingHistory];
      let added = 0;
      importHistory.forEach(day => {
        const exists = merged.some(d => d.ts === day.ts);
        if (!exists) { merged.push(day); added++; }
      });
      // Sort newest first
      merged.sort((a, b) => b.ts - a.ts);
      // Keep max 90
      if (merged.length > 90) merged.splice(90);

      localStorage.setItem(HISTORY_KEY, JSON.stringify(merged));

      // Import today if present and current today is empty
      if (payload.today?.length && !trackerItems.length) {
        trackerItems = payload.today;
        saveToStorage();
        renderTracker();
      }
      ...
      // Import walk history
      if (payload.walkHistory?.length) {
        const existingWalk = getWalkHistory();
        const mergedWalk = [...existingWalk];
        payload.walkHistory.forEach(d => {
          if (!mergedWalk.some(e => e.ts === d.ts)) { mergedWalk.push(d); added++; }
        });
        mergedWalk.sort((a, b) => b.ts - a.ts);
        if (mergedWalk.length > 200) mergedWalk.splice(200);
        try { localStorage.setItem(WALK_HISTORY_KEY, JSON.stringify(mergedWalk)); } catch(e) {}
        renderWalkHistory();
      }

      showToast(`✓ Import erfolgreich – ${added} neue Einträge hinzugefügt`);
    } catch(err) {
      showToast('⚠ Fehler beim Lesen der Datei', 3000);
    }
